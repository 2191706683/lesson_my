<template>
  <div>
      ...
      <el-table :load="state.loading" :data="state.tableData" tooltip-effect="dark" style="width:100%"
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template #deafult="scope">
            <a style="margin-right:10px;" @click.prevent="handleEdit(scope.row.carouselId)">修改</a>
    ...
    <el-dialog v-model="state.showDialog" title="添加轮播图" width="400px">
      <el-form :model="state.swiperForm" :rules="state.rules" ref="formRef" label-width="100px" class="goods-form">
        <el-form-item label="图片" prop="url">
          <el-upload class="avatar-uploader" :action="state.uploadImgServer" accept="jpg,jpeg,png"
            :headers="{ token: state.token }" :on-success="handleUrlSuccess">
            <img :src="state.swiperForm.url" class="avatar" v-if="state.swiperForm.url"
